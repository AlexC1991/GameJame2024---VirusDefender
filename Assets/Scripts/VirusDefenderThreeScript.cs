...
namespace Batty251
{
    public class VirusDefenderThreeScript : MonoBehaviour
    {
        ...
        private GameObject[] bugChildrenObjects;
        private GameObject[] tileObjects;
        private Transform childrenInParentBug;
        private Transform childrenInParentTile;
        ...
        IEnumerator PercentageCalculateStart()
        {
            ...
            childrenInParentBug = bugContainer.transform;
            childrenInParentTile = tileParent.transform;
            bugChildrenObjects = GetAllChildren(childrenInParentBug);
            tileObjects = GetAllChildren(childrenInParentTile);
            ...
            foreach (var g in tileObjects)
            {
                g.GetComponent<SpriteRenderer>().color = slowBugsDown.tileSlowEffectColor;
            }
            yield return new WaitForSecondsRealtime(3f);
            foreach (var g in tileObjects)
            {
                g.GetComponent<SpriteRenderer>().color = slowBugsDown.originalTileColorEffect;
            }
        ...
        IEnumerator GetThemBugs()
        {
            ...
            foreach (var g in bugChildrenObjects)
            {
                g.GetComponent<DynamicHealthIndicator>().TakeDamage(60);
            }
